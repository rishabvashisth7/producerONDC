...
import com.infy.bapondc.config.EmbeddedKafka;
import com.infy.bapondc.config.KafkaSseConsumer;
import com.infy.bapondc.config.KafkaSseProducer;
...
class BapondcKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/bapondc-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/bapondc-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/bapondc-kafka/unregister"));
                return;
            }
        }
